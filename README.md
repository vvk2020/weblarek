...
- `set<K extends keyof IBuyer>(field: K, value: IBuyer[K])` - универсальный метод, принимающий имя (без префикса "_") и значение свойства покупателя, соответствующее интерфейсу `IBuyer` и задающий его соответствующему защищенному свойству (с префиксом "_");
