...
- `title` -  cеттером названия товара
- `price` - сеттером цены товара типа `Price`
- `id` - сеттером идентификатора товара

#### Карточка товара в галерее - `IGalleryCard`

Описывает карточку товара в галерее со следующими методами:

- `category` - сеттером категории товара
- `image` - сеттером изображения товара
...
- `errors(text: string)` - сеттер, устанавливающий текст ошибки на форме
- `disableSubmitButton(disabled: boolean)` - сеттер, устанавливающий статус доступности кнопки отправки формы
...
- `selectedPayment(button: HTMLButtonElement | undefined)` - сеттер, устанавливающий тип платежа
...
- `total(value: Price)` - сеттер, устанавливающий стоимость заказа
...
- `id(value: string)` - сеттер, задающий порядковый номер товара в корзине
...
- `cards(cards: HTMLElement[])`  - сеттер, заменяющий содержимое контейнера на массив карточек типа HTMLElement.
...
- `cards(cards: HTMLElement[])`  - сеттер, размещающий карточки товара в контейнере и стилизующий их список (если отсутствуют, то выводится сообщение "Корзина пуста" и блокируется кнопка оформления заказа)
- `total(value: Price)` - сеттер, задающий в представлении стоимость товаровв корзине
...
- `basketCounter(value: number)`  - сеттер, задающий в новое значение счетчика товаров в корзине
...
- `total(value: Price)` - сеттер, задающий стоимость заказа
