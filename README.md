...
Описывает абстрактную карточку товара, обладающую следующими методами:

- `set title(text: string)` -  cеттером названия товара
- `set price(value: Price)` - сеттером цены товара типа `Price`
- `set id(value: string)` - сеттером идентификатора товара

#### Карточка товара в галерее - `IGalleryCard`

Описывает карточку товара в галерее со следующими методами:

- `set category(key: string)` - сеттером категории товара
- `set image(path: string)` - сеттером изображения товара

#### Карточка товара в корзине - IBasketCard

Описывает карточку товара в корзине со следующими методами:

- `render(data?: IBasketCardData, index?: number)` - методом, возвращающим разметку карточки типа `IBasketCardData` с порядковым номером `index` в корзине

#### Header страницы - `IHeaderData`

Описывает Header страницы, имеющий следующие свойства:

- `basketCounter` - счетчик товаров в корзине

#### Тип структуры карточек товаров в галлерее, корзине и окне подробного просмотра - `IGalleryCardData`, `IBasketCardData` и `IPreviewCardData`

Разработаны на основе утилитарного типа `Pick`, отбирающего из интерефейса `IProduct` следющие свойств карточки товара:

- `IGalleryCardData` - `id`, `category`, `title`, `image` и `price`
- `IBasketCardData` - `id`, `category`, `title` и `price`
- `IPreviewCardData` - все свойства `IProduct`

#### Модальное окно - `IModal`

Описывает методы и свойства модального окна:

- `setСontent(elements: HTMLElement[])` - метод, размещающий контент в контейнере окна
- `open()` - метод открытия окна
- `close()` - метод закрытия окна

#### Галлерея карточек товара - `IGalleryData`

Описывает свойства галереи карточек товаров:

- `cards` - массив карточек товаров типа HTMLElement


#### Корзина карточек товара - `IBasketData`
  
  Описывает свойства корзины карточек товаров:

- `cards` - массив карточек товаров типа HTMLElement
- `total` - стоимость корзины типа `Price`

##### Абстрактная форма - `IForm`
  
  Описывает методы абстрактной формы оформления заказа:

- `set errors(text: string)` - сеттер, устанавливающий текст ошибки на форме
- `set disableSubmitButton(disabled: boolean)` - сеттер, устанавливающий статус доступности кнопки отправки формы
- `reset()` - метод сброса полей ввода данных (только `<input>`)

#### Первая форма оформления заказа - `IOrderForm`
  
  Описывает методы первой формы оформления заказа:

- `set selectedPayment(button: HTMLButtonElement | undefined)` - сеттер, устанавливающий тип платежа
- `reset()` - метод сброса полей ввода данных (в том числе и выбора способа оплаты)

#### Тип структуры первой формы оформления заказа - `IOrderFields`
  
  Описывает структуру первой формы оформления заказа:

- `payment` - HTMLButtonElement кнопка способа оплаты заказа
- `address` - HTMLInputElement адреса доставки
 
#### Тип структуры второй формы оформления заказа - `IContactsFields`
  
  Описывает структуру второй формы оформления заказа:

- `emailInput` - HTMLInputElement email
- `phoneInput` - HTMLInputElement телефона

#### Тип структуры сообщения об успешном формлении заказа - `ISuccess`
  
  Описывает структуру сообщения об успешном формлении заказа:

- `set total(value: Price)` - сеттер, устанавливающий стоимость заказа
...
- `get items(): IProduct[]` - получение массива товаров типа `IProduct`, хранимых в каталоге
- `set items(items: IProduct[])` - сохранение (перезапись) массива `items` товаров типа `IProduct` (перед сохранением выбранный товар сбрасывается в `undefined`)
- `get selectedItem(): IProduct | undefined` - получение выбранного тивара типа `IProduct` (если товар не определен, то возвращается `undefined`)  
- `set selectedItem(item: IProduct | undefined)` - сохранение выбранного товара `item`
...
- `get items(): IProduct[]` - получение массива товаров типа `IProduct`, хранимых в корзине
- `set items(items: IProduct[])` - сохранение массива `items` товаров типа `IProduct` в корзину (перед сохранением выполняется ее очистка)
...
- `get data(): IBuyer` - получение данных покупателя в виде объекта
...
- `set title(text: string)` - сеттер, задающий в карточке название товара
- `set price(value: Price)` - сеттер, задающий в карточке цену товара (если цена не указана, то выводится "Бесценно")
- `set id(value: string)` - сеттер, задающий порядковый номер товара в корзине
...
- `set category(key: string)` - сеттер, задающий в карточке категорию товара и ее стиль отображения
- `set image(path: string)` - сеттер, задающий в карточке изображение товара
...
#### КАРТОЧКА В ПОДРОБНОМ ОТОБРАЖЕНИИ - `PreviewCard`

##### Назначение

Создание карточки в подробном отображении.

##### Реализация

Расширяет класс `GalleryCard` карточки галереи.

##### Конструктор

Принимает контейнер карточки и экземпляр брокера событий. Определяет элементы размеки, отвечающие за описание и кнопку добавления/удаления товара из корзины. Добавляет обработчик события на кнопку добавления/удаления товара из корзины.

##### Поля

- `descriptionEl` - HTMLElement описания товара
- `basketBtn` - HTMLButtonElement кнопки добавления/удаления товара из корзины
- `hasItemInBasket` - флаг наличия карточки в корзине

##### Методы

- `render(data?: IPreviewCardData, hasItemInBasket?: boolean): HTMLElement` - public-метод, отвечающий за создание HTML-разметки карточки в подробном отображении. Опционально примает данные типа `IPreviewCardData` и флаг наличия карточки в корзине. Возвращает HTML-разметку карточки корзины с обновленными данными или ее контейнер.
...
- `set errors(text: string)` - сеттер, задающий ошибку валидации формы
- `set disableSubmitButton(disabled: boolean)` - сеттер, определяющий доступность кнопки отправки формы
...
- `set selectedPayment(button: HTMLButtonElement | undefined)`  - сеттер, задающий способ оплаты заказа
...
#### ГАЛЕРЕЯ КАРТОЧЕК ТОВАРОВ - `GalleryView`

##### Назначение

Создание галереи карточек товаров.

##### Реализация

Расширяет класс `Component`, специализированный под интерфейс `IGalleryData`.

##### Конструктор

Принимает контейнер галереи товаров.

##### Методы

- `set cards(cards: HTMLElement[])`  - сеттер, заменяющий содержимое контейнера на массив карточек типа HTMLElement.

#### КОРЗИНА КАРТОЧЕК ТОВАРОВ - `BasketView`

##### Назначение

Создание корзины карточек товаров.

##### Реализация

Расширяет класс `Component`, специализированный под интерфейс `IBasketData`.

##### Конструктор

Принимает контейнер корзины и экземпляр брокера событий. Определяет элементы размеки, отвечающие за контейнер карточек товара, стоимость товаров в корзине и отправку формы. Добавляет обработчик события выбора способа оплаты и изменения в поле ввода адреса доставки.

##### Поля

- `listEl` - HTMLUListElement контейнера карточек товара
- `totalEl` - HTMLElement стоимости товаров в корзине
- `orderBtn` - HTMLButtonElement кнопки оформления заказа

##### Методы

- `set cards(cards: HTMLElement[])`  - сеттер, размещающий карточки товара в контейнере и стилизующий их список (если отсутствуют, то выводится сообщение "Корзина пуста" и блокируется кнопка оформления заказа)
- `set total(value: Price)` - сеттер, задающий в представлении стоимость товаровв корзине

#### HEADER СТРАНИЦЫ - `Header`

##### Назначение

Создание представления Header-элементов.

##### Реализация

Расширяет класс `Component`, специализированный под интерфейс `IHeaderData`.

##### Конструктор

Принимает контейнер header'а и экземпляр брокера событий. Определяет элементы размеки, отвечающиq за счетчик товаров в корзине. Добавляет обработчик события открытия корзины.

##### Поля

- `basketCounterEl` - HTMLElement счетчика товаров в корзине

##### Методы

- `set basketCounter(value: number)`  - сеттер, задающий в новое значение счетчика товаров в корзине

#### МОДАЛЬНОЕ ОКНО - `Modal`

##### Назначение

Создание модального окна-контейнера для контента.

##### Реализация

Расширяет класс `Component`, специализированный под заданный интерфейс. Реализует интерфейс `IModal`.

##### Конструктор

Принимает контейнер модального окна, экземпляр брокера событий и контент типа `HTMLElement[]`, размещаемый в нем. Определяет элементы размеки, отвечающие за контейнер контета и кнопку закрытия окна. Размещает контет в контейнере окна и добавляет обработчики событий закрытия окна.

##### Поля

- `closeBtn` - HTMLButtonElement кнопки закрытия окна
- `contentContainer` - HTMLElement-контейнер контента

##### Методы

- `setСontent(elements: HTMLElement[])` - public-метод размещения контента в контейнере
- `open()` - public-метод открытия окна
- `close()` - public-метод закрытия окна

#### СООБЩЕНИЕ ОБ УСПЕШНОМ ОФОРМЛЕНИИ ЗАКАЗА - `Success`

##### Назначение

Создание контента сообщения об успешном оформлении заказа.

##### Реализация

Расширяет класс `Component`, специализированный под интерфейс `ISuccess`.

##### Конструктор

Принимает контейнер сообщения и экземпляр брокера событий. Определяет элементы размеки, отвечающие за стоимость заказа и кнопку закрытия окна. Добавляет обработчики событий закрытия окна.

##### Поля

- `totalEl` - HTMLElement стоимости заказа
- `closeBtn` - HTMLButtonElement кнопки закрытия окна

##### Методы

- `set total(value: Price)` - сеттер, задающий стоимость заказа

### События

Для минимизации количества ошибок, возникающих при использовании имен событий, введена константа `EVENTS`, хранящая имена событий брокера.

#### Модель данных

- `EVENTS.basket.change` - изменение корзины
- `EVENTS.catalog.change` - изменение каталога товаров

#### Представление

- класс `Header`:
  - `EVENTS.basket.openModal` - открытие корзины в модальном окне

- класс `Card`:
  - `EVENTS.card.preview` - выбор карточки для подробного просмотра
...
- класс `Form`:
  - `EVENTS.forms[formName].submit` - отправка формы с именем `formName`

- класс `OrderForm`
  - `EVENTS.forms.order.chahgeFields` - изнение в поле ввода данных первой формы оформлеия заказа

- класс `ContactsForm`
  - `EVENTS.forms.contacts.chahgeFields` - изнение в поле ввода данных второй формы оформлеия заказа
  
- класс `BasketView`:
  - `EVENTS.forms.order.open` - открытие первой формы оформления заказа

- класс `Success`:
  - `EVENTS.success.close` - закрытие модального окна с с сообщением об успешном оформлении заказа

### Презентер

####

Разработанное приложение относительно простое, поэтому презентер, отвечающий за всю логику его работы, реализован в основном скрипте приложения `main.ts`.

В презентере:

- определены необходимые элементы верстки (шаблоны карточек, форм и других компонентов, контейнеры галереи, корзины, модального окна и header)
- созданы и инициализированы экземпляры всех необходимых классов
- загружены данные галереи с WEB-сервера
- в брокере зарегистрированы все необходимые обработчики событий



  ### UML-диаграама приложения

- [PNG](./src/public/app-uml.png)
- [SVG](./src/public/app-uml.svg)
